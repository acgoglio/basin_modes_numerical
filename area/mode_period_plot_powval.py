...
for idx_gp, gp in enumerate(group_centers):
    ...
    plt.contour(nav_lon, nav_lat, masked_pow_pct, levels=[0.000], colors="magenta", linewidths=1.0)
    plt.contourf(nav_lon, nav_lat, tmask, levels=[-1000, 0.05], colors="gray")
    plt.contour(nav_lon, nav_lat, tmask, levels=[0.05], colors="black", linewidths=0.8)
    ...
    plt.title(f"Absolute Energy for Mode with Period: {gp:.1f} h ± {uncertainty} h")
    plt.xlabel("Longitude")
    plt.ylabel("Latitude")
    plt.xlim(-6, 36.3)
    plt.ylim(30, 46)
